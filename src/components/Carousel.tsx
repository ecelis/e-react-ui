...
export declare interface ISlider
  extends ThemedStyledFunction<"div", any, {}, never> {
  /** Direction */
  direction?: string;
    ...
    <Slider>
      {props.items.map((photo, idx) => {
        return (
          ...
          >
            <img id={idx} key={idx} src={photo.src} alt={photo.alt} />
          </Slide>
        );
      })}
      <CarouselButton direction="right" onClick={nextHandler}>
        {">"}
      </CarouselButton>
      <CarouselButton direction="left" onClick={prevHandler}>
        {"<"}
      </CarouselButton>
    </Slider>
  );
